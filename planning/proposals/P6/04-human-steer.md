...
**Authority:** SD v1.3 §7.3（governance primitives + quiescence）, §8.1; S2 全部（尤 §3–§7）; DID v1.9 §4.2, §12.10 表（SteerWork→WorkSteered）, §2.3（WorkRevision）, §3.4（quiescence）; P2 `01` §6（StopExecution）.
...
DID §12.10 表行：`RefineWork / semantic SteerWork → WorkRefined / WorkSteered`，precondition = authority + revision，效果 = `Open, revision++`（P6 只实现 SteerWork 侧；RefineWork 属 producer 自refine，语义同 revision 通道，一并由本 handler 家族承载）。
...
| Condition | Rejection |
|---|---|
| work not found / 不属于 workspace | `CommandRejection.WorkNotFound`（若 P1 未定义则 P6 冻结） |
...
- No `CancelWork` / `CompleteWork` 语义（前者 P1 命令族后续阶段接线，后者 P8）。
