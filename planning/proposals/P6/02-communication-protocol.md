...
**Authority:** DID v1.9 §4.2, §1.8(Message), §2 表(Inbox), §5.4, §12.10 表, §8.4A; SD v1.3 §7.1–§7.5, §13.5, 不变量 20/21/31; P3 `03` §3.
**Status:** PROPOSAL DRAFT.

## 1. P6 的 Message 语义子集

work-plane primitives 共 6 种（SD §7.2）；P6 冻结其中 4 种为 Model-visible Message kinds（open decision D2）：
...
| bodyRef 超过 sender 上传配额 | `CommandRejection.ResourceExhausted`（若 P1 未定义则 P6 冻结此枚举） |
...
验证来源、authority、结构并持久化。进入 recipient Workspace Inbox。
...
（其余 P7：Deliverable→Dependency satisfaction）
```

Promotion 不调用模型（Receive != Promote != Consume）。
...
```text
GovernanceRequest { kind: FormationApproval, proposal, correlationId }
  → human Inbox（01 §4 第一层 gate）
